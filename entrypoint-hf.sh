...
if [ -d /app/config/docs ] && [ -z "$(ls -A "$WORK/docs/guides" 2>/dev/null)" ]; then
  cp -r /app/config/docs/* "$WORK/docs/" 2>/dev/null || true
  echo "[entrypoint] Seeded docs library from config/docs"
fi
