...
test('OAI-07: prompt > 100KB rejected', async () => {
  await withServer(startOaiApp(), async ({ port }) => {
    const r = await req(port, 'POST', '/v1/chat/completions', {
      model: 'claude-sonnet-4-6', messages: [{ role: 'user', content: 'x'.repeat(100001) }],
    });
...
test('OAI: no user message rejected', async () => {
  await withServer(startOaiApp(), async ({ port }) => {
    const r = await req(port, 'POST', '/v1/chat/completions', {
      model: 'claude-sonnet-4-6', messages: [{ role: 'system', content: 'hi' }],
    });
    assert.equal(r.status, 400);
  });
});
...
test('OAI-02: non-streaming completion success', async (t) => {
  t.mock.method(childProcess, 'execFile', (_c, _a, _o, cb) => cb(null, 'Test response from Claude'));
  await withServer(startOaiApp(), async ({ port }) => {
    const r = await req(port, 'POST', '/v1/chat/completions', {
...
test('OAI-04: bp: model prefix routes to session', async (t) => {
  const capturedArgs = [];
  t.mock.method(childProcess, 'execFile', (_c, args, _o, cb) => { capturedArgs.push(args); cb(null, 'response'); });
  await withServer(startOaiApp(), async ({ port }) => {
    await req(port, 'POST', '/v1/chat/completions', {
      model: 'bp:my-session-id', messages: [{ role: 'user', content: 'test' }],
    });
