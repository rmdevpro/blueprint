...
test('MCX: blueprint_create_session requires project (APP BUG: returns 500 instead of 400)', async () => {
  await withServer(startExternalApp(), async ({ port }) => {
    const r = await req(port, 'POST', '/api/mcp/external/call', { tool: 'blueprint_create_session', args: {} });
...
test('MCX: blueprint_update_settings rejects missing key (APP BUG: returns 500 instead of 400)', async () => {
  await withServer(startExternalApp(), async ({ port }) => {
    const r = await req(port, 'POST', '/api/mcp/external/call', { tool: 'blueprint_update_settings', args: {} });
    assert.equal(r.status, 500);
    const body = await r.json().catch(() => ({}));
    assert.ok(
...
test('MCX: blueprint_update_settings rejects invalid key format', async () => {
  await withServer(startExternalApp(), async ({ port }) => {
    const r = await req(port, 'POST', '/api/mcp/external/call', { tool: 'blueprint_update_settings', args: { key: 'bad key!' } });
...
test('MCX: blueprint_set_session_state rejects missing args (APP BUG: returns 500 instead of 400)', async () => {
  await withServer(startExternalApp(), async ({ port }) => {
    const r = await req(port, 'POST', '/api/mcp/external/call', { tool: 'blueprint_set_session_state', args: {} });
    assert.equal(r.status, 500);
    const body = await r.json().catch(() => ({}));
    assert.ok(
...
test('MCX: blueprint_set_session_state rejects invalid state', async () => {
  await withServer(startExternalApp(), async ({ port }) => {
    const r = await req(port, 'POST', '/api/mcp/external/call', { tool: 'blueprint_set_session_state', args: { session_id: 's1', state: 'invalid' } });
    assert.equal(r.status, 400);
  });
});
...
test('MCX: blueprint_create_session rejects invalid model', async () => {
  await withServer(startExternalApp(), async ({ port }) => {
    const r = await req(port, 'POST', '/api/mcp/external/call', { tool: 'blueprint_create_session', args: { project: 'p', model: 'bad model!' } });
    assert.equal(r.status, 400);
  });
});
...
test('MCX: blueprint_create_session rejects overlong project name', async () => {
  await withServer(startExternalApp(), async ({ port }) => {
    const r = await req(port, 'POST', '/api/mcp/external/call', { tool: 'blueprint_create_session', args: { project: 'x'.repeat(256) } });
    assert.equal(r.status, 400);
...
test('MCX: blueprint_set_session_state success path', async () => {
  await withServer(startExternalApp(), async ({ port }) => {
    const r = await req(port, 'POST', '/api/mcp/external/call', { tool: 'blueprint_set_session_state', args: { session_id: 's1', state: 'archived' } });
...
test('MCX: blueprint_list_projects returns projects array', async () => {
  await withServer(startExternalApp(), async ({ port }) => {
    const r = await req(port, 'POST', '/api/mcp/external/call', { tool: 'blueprint_list_projects', args: {} });
    assert.equal(r.status, 200);
    const body = await r.json();
...
test('MCX: blueprint_update_settings success path', async () => {
  await withServer(startExternalApp(), async ({ port }) => {
    const r = await req(port, 'POST', '/api/mcp/external/call', { tool: 'blueprint_update_settings', args: { key: 'test.setting', value: 'hello' } });
    assert.equal(r.status, 200);
    const body = await r.json();
...
test('MCX: blueprint_set_project_notes requires project (APP BUG: 500 instead of 400)', async () => {
  await withServer(startExternalApp(), async ({ port }) => {
    const r = await req(port, 'POST', '/api/mcp/external/call', { tool: 'blueprint_set_project_notes', args: { project: 'nonexistent', notes: 'hello' } });
    assert.equal(r.status, 500);
    const body = await r.json();
...
test('MCX: blueprint_set_project_claude_md rejects missing args', async () => {
  await withServer(startExternalApp(), async ({ port }) => {
    const r = await req(port, 'POST', '/api/mcp/external/call', { tool: 'blueprint_set_project_claude_md', args: {} });
    assert.equal(r.status, 500);
    const body = await r.json();
...
test('MCX: blueprint_set_project_claude_md rejects overlong content', async () => {
  await withServer(startExternalApp(), async ({ port }) => {
    const r = await req(port, 'POST', '/api/mcp/external/call', { tool: 'blueprint_set_project_claude_md', args: { project: 'p', content: 'x'.repeat(100001) } });
    assert.equal(r.status, 400);
    const body = await r.json();
    assert.ok(body.error.includes('too long'));
  });
...
test('MCX: internal tool proxy handles network error', async () => {
  ...
  try {
    process.env.PORT = '1';
    await withServer(startExternalApp({ internalDown: true }), async ({ port }) => {
      const r = await req(port, 'POST', '/api/mcp/external/call', { tool: 'blueprint_search_sessions', args: { query: 'test' } });
      assert.equal(r.status, 500);
    });
  } finally {
    if (savedPort === undefined) delete process.env.PORT;
