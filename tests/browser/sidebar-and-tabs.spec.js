...
describe('sidebar and tabs (browser)', () => {
  ...
  it('BRW-01: page loads with sidebar containing project data from server', async () => {
    ...
    assert.ok(await page.locator('#sidebar-header h1').isVisible(), 'Sidebar header must be visible');
    // Behavioral: verify loadState() actually populated the sidebar with data from /api/state
    const projectCount = await page.locator('#project-list .project-entry, #project-list [data-project]').count();
    // Gray-box: fetch /api/state directly and compare
    const apiState = await page.evaluate(async () => {
      const r = await fetch('/api/state');
      return r.json();
    });
    assert.equal(projectCount, apiState.projects.length,
  ...
  it('BRW-06: filter buttons switch active state and filter the session list', async () => {
    ...
    assert.ok(activeCount <= allCount,
  ...
  it('BRW-07: sort dropdown changes actual session order', async () => {
    ...
    const nameOrder = await page.locator('.session-entry .session-name, [data-session] .session-name').allTextContents();
    await page.locator('#session-sort').selectOption('messages');
    assert.equal(await page.locator('#session-sort').inputValue(), 'messages');
    const msgOrder = await page.locator('.session-entry .session-name, [data-session] .session-name').allTextContents();
    ...
    if (nameOrder.length >= 2) {
      ...
      const currentOrder = await page.locator('.session-entry .session-name, [data-session] .session-name').allTextContents();
      assert.equal(currentOrder.length, nameOrder.length,
  ...
  it('UI-05: session search filters the displayed session list', async () => {
    ...
    assert.equal(restoredCount, beforeCount,
